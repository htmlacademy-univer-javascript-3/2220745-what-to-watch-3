...
import { ALL_GENRES, ApiRoute, AuthorizationStatus, FilmStatus, NameSpace } from '../const.ts';
import {
  checkAuthAction,
  fetchFilmDataAction,
  fetchFilmsAction,
...
describe('Async actions', () => {
  ...
  describe('setFilmStatus', () => {
    it('should dispatch "setFilmStatus.pending" and "setFilmStatus.fulfilled" with thunk "setFilmStatus', async () => {
      ...
      mockAxiosAdapter
        .onPost(ApiRoute.SetFilmStatus(film.id, FilmStatus.Vied))
        .reply(200, { ...film, isFavorite: false });

      await store.dispatch(setFilmStatusAction({ id: film.id, filmStatus: FilmStatus.Vied }));
      const emittedActions = store.getActions();
      const extractedActionsTypes = extractActionsTypes(emittedActions);
    ...
    it('should dispatch "setFilmStatus.pending" and "setFilmStatus.rejected" with thunk "setFilmStatus" when server response 400', async () => {
      const id = 'unknown';
      const filmStatus = FilmStatus.Vied;
